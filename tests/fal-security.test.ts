...
describe("FAL secret boundary", () => {
  ...
  it("returns only a masked status and keeps provider errors and logs free of the key", async () => {
    ...
    const status = await createFalClient(
      { apiKey: syntheticSentinel, baseUrl: "https://api.fal.ai", queueBaseUrl: "https://queue.fal.run", syncBaseUrl: "https://fal.run", timeoutMs: 10, submitTimeoutMs: 10, downloadTimeoutMs: 10 },
