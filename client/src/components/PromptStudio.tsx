...
import { AlertTriangle, Check, GitCompareArrows, RotateCcw, WandSparkles } from "lucide-react";
import { trpc } from "@/lib/trpc";
import { EmptyState, ErrorState, LoadingState } from "./States";

export default function PromptStudio({ projectId }: { projectId: string }) {
  ...
  const [generationModel, setGenerationModel] = useState("GPT Image 1.5");
  const [generationEndpoint, setGenerationEndpoint] = useState("fal-ai/gpt-image-1.5");
  ...
  const activePagePlanId = pagePlanId || pages.data?.[0]?.id || "";
  ...
  return (
    <div className="mt-8 space-y-6">
      <div className="rounded-[24px] border border-[var(--line)] bg-[var(--paper-strong)] p-5 shadow-[0_12px_38px_rgba(32,51,72,.05)] md:p-7">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3"><label className="block"><span className="mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted-ink)]">Page plan</span><select value={activePagePlanId} onChange={(event) => { setPagePlanId(event.target.value); setLeftVersionId(""); setRightVersionId(""); }} className="mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 py-3 text-sm text-[var(--ink)]">{pages.data.map((page) => <option key={page.id} value={page.id}>Page {page.pageNumber} — {page.sceneDirection || "Untitled scene"}</option>)}</select></label><label className="block"><span className="mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted-ink)]">Model label</span><input value={generationModel} onChange={(event) => setGenerationModel(event.target.value)} className="mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 py-3 text-sm text-[var(--ink)]" /></label><label className="block"><span className="mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted-ink)]">Endpoint identifier</span><input value={generationEndpoint} onChange={(event) => setGenerationEndpoint(event.target.value)} className="mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 py-3 text-sm text-[var(--ink)]" /></label></div>
        <div className="mt-4 grid gap-4 md:grid-cols-3"><label className="block"><span className="mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted-ink)]">Aspect ratio</span><select value={aspectRatio} onChange={(event) => setAspectRatio(event.target.value)} className="mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 py-3 text-sm text-[var(--ink)]"><option value="1:1">1:1 square</option><option value="3:2">3:2 landscape</option><option value="2:3">2:3 portrait</option></select></label><label className="block"><span className="mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted-ink)]">Seed (optional)</span><input value={seed} onChange={(event) => setSeed(event.target.value.replace(/[^0-9-]/g, ""))} placeholder="Provider default" className="mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--paper)] px-3 py-3 text-sm text-[var(--ink)]" /></label><div className="rounded-xl border border-[var(--line)] bg-[#f8f5ec] px-3 py-3"><p className="mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted-ink)]">Current scene</p><p className="mt-2 text-sm font-semibold text-[var(--ink)]">{currentPage?.sceneDirection || "No scene direction yet"}</p></div></div>
