...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] ${copy.tone}`}><CircleDashed size={12} aria-hidden="true" />{copy.label}</span>;
}
...
export default function PageGenerationStudio({ projectId }: { projectId: string }) {
  ...
  const submitPage = async (kind: "initial" | "variation" | "prompt_edit", sourceAssetId?: string, useEdit = false) => {
    ...
    try {
      ...
      setFeedback(`Page ${selectedPage?.pageNumber ?? "selected"}: ${error instanceof Error ? error.message : "The generation request could not be submitted."}`);
    }
  };
  ...
  return <div className="space-y-7"><p className="sr-only" role="status" aria-live="polite" aria-atomic="true">Page {selectedPage?.pageNumber ?? "selected"} status: {statusCopy[currentStatus as StudioStatus]?.label ?? currentStatus}.</p>
    ...
    <ConfirmDialog open={dangerAction === "stop"} title={`Stop generation for page ${selectedPage?.pageNumber ?? "selected"}?`} description="This asks the provider to cancel the in-progress generation. Provider work may already be billed and cannot always be reversed." confirmLabel="Stop generation" tone="warning" onCancel={() => setDangerAction(null)} onConfirm={() => { if (activeJob) cancel.mutate({ jobId: activeJob.id }); setDangerAction(null); }} />
    <ConfirmDialog open={dangerAction === "archive"} title={`Archive the approved asset for page ${selectedPage?.pageNumber ?? "selected"}?`} description="This removes the approved variant from the active review path while preserving its lineage. You can no longer use it as the current approved page asset." confirmLabel="Archive asset" onCancel={() => setDangerAction(null)} onConfirm={() => { if (latestAsset) review.mutate({ assetId: latestAsset.id, decision: "archived" }); setDangerAction(null); }} />
  </div>;
}
